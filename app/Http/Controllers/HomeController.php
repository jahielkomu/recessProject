...
//  two alternatives to present the percentage choice any;
        ...
        $data = myviews::select(
            ...
            ,\DB::raw("total")
             // \DB::raw("COALESCE((LEAD(total) OVER (ORDER BY months DESC)-total)/total, 0) Percent_Change"s)
        )
...
<<<<<<< HEAD
        $district_list=DB::select(DB::raw('SELECT DATE_FORMAT(date, "%Y-%m") AS month FROM treasuries
...
<<<<<<< HEAD
    public function showcharts(Request $request){
      $mydata=DB::select(DB::raw('SELECT amount,source From treasuries WHERE DATE_FORMAT(date, "%y-%m")="'.$request->month.'"'));
      //$value=array();
      //$source=array();
      $array[]=[];
